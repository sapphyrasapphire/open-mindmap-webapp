 ...
 *
 * https://github.com/sapphyrasapphire/open-mindmap-webapp
 * 
...
function loadNodeData(data) {
  ...
  for (const entry of json) {
    ...
    new_node.x = parseInt(entry.x);
    new_node.y = parseInt(entry.y);
...
function loadNodeDataFromFile() {
  ...
  if (files[0].type != "application/json") {
    console.log("Invalid file data for import.");
    return;
  }
...
function showAboutBox() {
  ...
  if (!dialog) {
    ...
    titlebar.className = "heading";
    titlebar.innerText = "About Open Mindmap Webapp";

    content.innerHTML = `<p>Version: ${VERSION}<br>
    Version Date: ${VERSION_DATE}</p>
    <p>
    <a href="https://github.com/sapphyrasapphire/open-mindmap-webapp">Github Page</a><br>
    ...
    let close_button = $n("button");

    close_button.innerText = "close";
    close_button.onclick = (e) => $("#about_box").close();
...
function setupInfobar() {
  ...
  infobar.innerHTML = `<i>Version ${VERSION}</i><br><a href="#" onclick="showKeyboardShortcuts();">Keyboard shortcuts</a><br>
    <a href="#" onclick="showAboutBox();">About this program</a>`;
}
...
function setupEventListeners() {
  ...
  window.addEventListener('mousemove', (e) => {
	...
	node.x = e.clientX - Math.floor(element.offsetWidth / 2);
	node.y = e.clientY - Math.floor(element.offsetHeight / 2);
